  ...
  useEffect(() => {
    if (contentRef.current) {
      ...
      for (let i = 0; i < numPages; i++) {
        ...
        const end = Math.min((i + 1) * pageHeight, contentHeight);
        newPages.push(
          <div
            key={i}
            style={{
          ...
          >
            <ResumeContent
              ...
              education={resumeData.education.slice(start, end)}
              experience={resumeData.experience.slice(start, end)}
              skills={resumeData.skills.slice(start, end)}
            />
          </div>
        );
      }
